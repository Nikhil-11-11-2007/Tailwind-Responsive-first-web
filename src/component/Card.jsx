const Card = ({elem,id}) => {
    ...
    return (
        <div style={{borderBottomLeftRadius:`${ id == 1 ? "50%": "2vw"}`, border:`${ id == 1 ? "none": ""}`, backgroundColor:`${ id == 1 ? "#8c00ff": ""}`}}
         className=" card h-[55vh] hover:scale-110 transition-all cursor-pointer duration-400 relative rounded-2xl w-full sm:w-[40%] lg:w-[28%] p-5 py-5 flex flex-col justify-between border-2 border-purple-600">
            <div className="flex items-center p-2 pb-5 border-purple-700 border-b-2 justify-between">
                ...
                <i style={{position:`${ id == 1 ? "absolute": ""}`, bottom: `${ id == 1 ? "0": ""}`, padding:`${ id == 1 ? "1.5vw": ""}`, }}
                 className="px-[1.5vw] py-[1vw] text-[calc(1vw+1rem)] rounded-[50%] bg-[#8c00ff] ri-arrow-right-up-long-line"></i>
            </div>
